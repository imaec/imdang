...
import info.imdang.app.model.insight.ExchangeItem
...
@HiltViewModel
open class InsightDetailViewModel @Inject constructor(
    ...
    val insightId = savedStateHandle["insightId"] ?: ""
    ...
    protected val _insight = MutableStateFlow(InsightDetailVo.init())
    val insight = _insight.asStateFlow()

    protected val _insightDetails = MutableStateFlow<List<InsightDetailItem>>(emptyList())
    val insightDetails = _insightDetails.asStateFlow()

    protected val _exchangeCoupon = MutableStateFlow(CouponVo.init())
    ...
    protected val _exchangeItems = MutableStateFlow<PagingData<ExchangeItem>>(PagingData.empty())
    val exchangeItems = _exchangeItems.asStateFlow()

    protected val _selectedExchangeItem = MutableStateFlow<ExchangeItem?>(null)
                ...
                if (insight.value.insightDetailStatus == InsightDetailStatus.EXCHANGE_COMPLETE ||
                    ...
                    if (infra != null &&
                        ...
                        listOf(
                            ...
                            InsightDetailItem.ComplexEnvironment(complexEnvironment),
                            InsightDetailItem.ComplexFacility(complexFacility),
